...
-- Ren2 and all of their bills/attendance. Run only after confirming the name.
do $$
declare
...
begin
  ...
  from public.staff
  where lower(name_en) = 'ren2';

  if target_count = 0 then
    raise exception 'No staff account named Ren2 was found.';
  end if;
  if target_count > 1 then
    raise exception 'More than one account is named Ren2; stop and choose by staff id.';
  end if;
  ...
  -- Put consumed materials back before removing the audit movements.
  ...
  -- Break references that would otherwise prevent deleting the bills.
  ...
  -- Preserve non-bill finance/stock records but detach the deleted employee.
  ...
  -- Recalculate customer totals from the bills that remain.
  ...
  -- Remove the matching Supabase Auth account so the login name can be reused.
